...
// Inline the scraping functions from utah_repeater_scraper_v2.js
async function scrapeRawRepeaterData() {
    ...
    try {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const repeater_data = processRawRepeaterData(row);
...
function processRawRepeaterData(row) {
    ...
    if (repeater.output_frequency) {
        try {
            ...
            repeater.band_name = getBandName(freq);
        } catch (error) {
            repeater.frequency = repeater.output_frequency;
        }
    }
...
function saveToCsvV2(repeaters) {
